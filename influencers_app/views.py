...
class InfluencersNameSearchMixin:
    def get_queryset(self):
        ...
        influencers_name_search = self.request.GET.get("influencers_name_search")
...
class ContentView(LoginRequiredMixin, ListView):
    ...
    def get_context_data(self, object_list=None, **kwargs):
        ...
        video_data_dict = Content.objects.select_related("channel_name").aggregate(
            Sum("number_of_views"),
            Sum("number_of_comments"),
            Sum("number_of_likes"),
            Sum("number_of_dislikes"),
            Avg("number_of_views"),
        )
        influencer = self.request.GET.get("influencer")
        context["videos"] = videos
        context["sum_views"] = video_data_dict.get("number_of_views__sum")
        context["sum_comments"] = video_data_dict.get("number_of_comments__sum")
        context["sum_likes"] = video_data_dict.get("number_of_likes__sum")
        context["sum_dislikes"] = video_data_dict.get("number_of_dislikes__sum")
        context["avg"] = video_data_dict.get("number_of_views__avg")
        if influencer:
            filtered_video_data_dict = (
                Content.objects.filter(channel_name__name=influencer)
            ...
            context["sum_views"] = filtered_video_data_dict.get("number_of_views__sum")
            context["sum_comments"] = filtered_video_data_dict.get(
                "number_of_comments__sum"
            )
            context["sum_likes"] = filtered_video_data_dict.get("number_of_likes__sum")
            context["sum_dislikes"] = filtered_video_data_dict.get(
                "number_of_dislikes__sum"
            )
            context["avg"] = filtered_video_data_dict.get("number_of_views__avg")
        return context
...
class ContentCreateView(CreateView):
    ...
    def form_valid(self, form):
        ...
        request = youtube.videos().list(part="statistics, snippet", id=video_url)
...
class ChartView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        video_information_by_month = Content.objects.annotate(
            month=TruncMonth("date_of_publication")
        ).values("month")
